...
const toolRegistry: Record<string, (...args: unknown[]) => Promise<string> | string> = {
  ...
  browser_click: (selector) => tools.browserClick(String(selector)),
  browser_type: (selector, text) => tools.browserType(String(selector), String(text)),
  browser_screenshot: () => tools.browserScreenshot(),
  browser_get_text: () => tools.browserGetText(),
  browser_scroll: (direction, amount) => tools.browserScroll(String(direction), amount ? Number(amount) : undefined),
  browser_page_info: () => tools.browserPageInfo(),
  browser_evaluate: (code) => tools.browserEvaluate(String(code)),
  browser_wait_for: (selector, timeout) => tools.browserWaitFor(String(selector), timeout ? Number(timeout) : undefined),
